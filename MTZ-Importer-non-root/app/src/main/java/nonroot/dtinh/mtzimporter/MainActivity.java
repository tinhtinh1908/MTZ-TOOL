...
public final class MainActivity extends Activity {
    ...
    private static final int REQUEST_SHIZUKU = 1101;
    private static final int REQUEST_NOTIFICATIONS = 1102;
    private static final int REQUEST_SHIZUKU_FRAMEWORK = 1103;
    private static final String SHIZUKU_PERMISSION =
    ...
    private TextView protectionDescription;
    private TextView protectionToggle;
    ...
    private View buildUiSafe() {
        ...
        rootBadge.setOnClickListener(v -> requestShizukuAccess());
    ...
    private LinearLayout createProtectionCard() {
        ...
        protectionTitle = makeText(
                "Đang kiểm tra bảo vệ",
                15,
                true,
                textPrimary
        );
        ...
        protectionToggle = makeText(
                "BẬT",
                12,
                true,
                successColor
        );
        protectionToggle.setGravity(Gravity.CENTER);
        protectionToggle.setOnClickListener(
                v -> setProtectionEnabled(
                        !ProtectionState.isEnabled(this)
                )
        );
        header.addView(
                protectionToggle,
        ...
        protectionDescription = makeText(
                "Giữ chủ đề đã nhập ổn định trên thiết bị.",
                12,
                false,
                textSecondary
        );
        protectionDescription.setLineSpacing(0f, 1.12f);
        protectionDescription.setPadding(0, dp(5), 0, dp(11));
        card.addView(protectionDescription);
        ...
        TextView autostart = makeProtectionAction("Tự khởi động");
        ...
        TextView battery = makeProtectionAction("Không giới hạn pin");
        battery.setOnClickListener(v -> requestBatteryExemption());
        LinearLayout.LayoutParams batteryParams =
                new LinearLayout.LayoutParams(0, dp(42), 1f);
        batteryParams.leftMargin = dp(8);
        actions.addView(battery, batteryParams);

        refreshProtectionUi();
        return card;
    }

    private TextView makeProtectionAction(String label) {
        ...
        action.setBackground(
                roundedBackground(withAlpha(accentColor, 24), 13)
        );
        return action;
    }
    ...
    private void checkShizukuAsync() {
        final boolean binderAvailable =
                RootShell.isBinderAvailable();
        final boolean permissionGranted =
                RootShell.isPermissionGranted();
        runOnUiThread(() -> {
            if (rootBadge == null) {
                return;
            }
            ...
            if (permissionGranted) {
                label = "Shizuku sẵn sàng";
                color = successColor;
            } else if (binderAvailable) {
                label = "Cấp quyền Shizuku";
                color = accentColor;
            } else {
                label = "Shizuku chưa chạy";
                color = errorColor;
            }
            rootBadge.setText(label);
            rootBadge.setTextColor(color);
            rootBadge.setBackground(
                    roundedBackground(withAlpha(color, 34), 24)
            );
        });
    }
    ...
    private void requestShizukuAccess() {
        ...
        if (RootShell.isBinderAvailable()) {
            RootShell.requestPermission(REQUEST_SHIZUKU);
            return;
        }

        if (Build.VERSION.SDK_INT >= 23
                && checkSelfPermission(SHIZUKU_PERMISSION)
                != PackageManager.PERMISSION_GRANTED) {
            try {
                requestPermissions(
                        new String[]{SHIZUKU_PERMISSION},
                        REQUEST_SHIZUKU_FRAMEWORK
                );
                return;
            } catch (Throwable ignored) {
            }
    ...
    private void openShizukuManager() {
        try {
            ...
            if (launch != null) {
                ...
                Toast.makeText(
                        this,
                        "Hãy khởi động Shizuku rồi quay lại MTZ Tool",
                        Toast.LENGTH_LONG
                ).show();
                return;
            }
        ...
        Toast.makeText(
                this,
                "Chưa tìm thấy Shizuku. Hãy cài và khởi động Shizuku.",
                Toast.LENGTH_LONG
        ).show();
    }
    ...
    public void onRequestPermissionsResult(
        ...
        if (requestCode == REQUEST_SHIZUKU_FRAMEWORK) {
            ...
            Toast.makeText(
                    this,
                    granted
                            ? "Đã cấp quyền API. Hãy khởi động Shizuku."
                            : "Shizuku chưa cấp quyền cho MTZ Tool.",
                    Toast.LENGTH_LONG
            ).show();
            checkShizukuAsync();
            if (granted && !RootShell.isBinderAvailable()) {
                openShizukuManager();
            }
        }
    }
    ...
    private void startProtectionService() {
        ...
        try {
            Intent service = new Intent(this, BlockService.class);
            ...
            if (protectionToggle != null) {
                protectionToggle.postDelayed(
                        this::refreshProtectionUi,
                        350
                );
            }
        } catch (Throwable error) {
    ...
    private void setProtectionEnabled(boolean enabled) {
        ProtectionState.setEnabled(this, enabled);
        if (enabled) {
            startProtectionService();
            Toast.makeText(
                    this,
                    "Đã bật bảo vệ chủ đề",
                    Toast.LENGTH_SHORT
            ).show();
            if (protectionToggle != null) {
                protectionToggle.postDelayed(
    ...
    private void refreshProtectionUi() {
        if (protectionTitle == null
                || protectionDescription == null
                || protectionToggle == null) {
            return;
        }

        boolean enabled = ProtectionState.isEnabled(this);
        boolean running = BlockService.isRunning();
        int color = enabled ? successColor : errorColor;
        protectionToggle.setText(enabled ? "BẬT" : "TẮT");
        protectionToggle.setTextColor(color);
        protectionToggle.setBackground(
                roundedBackground(withAlpha(color, 34), 13)
        );

        if (!enabled) {
            protectionTitle.setText("Bảo vệ chủ đề đang tắt");
            protectionDescription.setText(
                    "Bảo vệ chống đặt lại đã dừng."
            );
            protectionTitle.setTextColor(errorColor);
        } else if (running) {
            protectionTitle.setText("Bảo vệ chủ đề đang hoạt động");
            protectionDescription.setText(
                    "Giữ chủ đề đã nhập ổn định trên thiết bị."
            );
            protectionTitle.setTextColor(successColor);
        } else {
            protectionTitle.setText("Bảo vệ chủ đề đang khởi động");
            protectionDescription.setText(
                    "Đang khởi động bảo vệ nền."
            );
            protectionTitle.setTextColor(accentColor);
        }
    }
    ...
    private void openAutostartSettings() {
        ...
        miuiAutostart.setComponent(new ComponentName(
                "com.miui.securitycenter",
                "com.miui.permcenter.autostart.AutoStartManagementActivity"
        ));
        if (tryStart(miuiAutostart)) {
            return;
        }

        Intent permissions = new Intent();
        permissions.setComponent(new ComponentName(
                "com.miui.securitycenter",
                "com.miui.permcenter.permissions.PermissionsEditorActivity"
        ));
        permissions.putExtra("extra_pkgname", getPackageName());
        if (tryStart(permissions)) {
            return;
        }

...
final class ThemeImporter {
    ...
    ImportResult importFromUri(Uri uri) throws Exception {
        ...
        if (externalCache == null) {
            throw new IllegalStateException(
                    "Không mở được bộ nhớ tạm dùng chung với Shizuku."
            );
        }
        ...
        try {
            ...
            try (ZipFile zip = new ZipFile(archive)) {
                ...
                progress("Đang nhập dữ liệu bằng Shizuku…");
                copyStagingWithRoot(
                        staging,
                        archive,
                        title
    ...
    private void ensureRoot() throws Exception {
        ...
        if (result.code != 0
                || !RootShell.isPrivilegedIdentity(result.output)) {
            throw new IllegalStateException(
                    "Shizuku chưa sẵn sàng hoặc chưa được cấp quyền.\n"
                            + result.output
            );
        }
    }
    ...
    private void copyStagingWithRoot(
        ...
        if (result.code != 0) {
            throw new IllegalStateException(
                    "Lệnh Shizuku nhập dữ liệu thất bại ("
                            + result.code
                            + "):\n"
                            + result.output
            );
